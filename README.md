...
This project was created to apply what I've learned from Dr. Angela Yu's Web Development Bootcamp on Udemy. I used the MERN stack (Mongo, Express, React, and Node) with React on the frontend, Node/Express on the backend, and MongoDB as the database for this project.
...
I researched quite a bit for suitable deployment services and stumbled upon a few candidates, such as Heroku, Railway, and Vercel. I decided to settle with Vercel as they offer a free Hobby plan. The deployment encountered some problems at first, as Vercel had a certain rule of repository structure for backend deployments. After the backend deployment is successful, I replaced all of the url for data fetches in the frontend to point towards the deployed backend url, essentially from `localhost:port` to `tasty-backend.vercel.app`. Finally, I deployed the frontend and have made some minor bug fixes since then.
